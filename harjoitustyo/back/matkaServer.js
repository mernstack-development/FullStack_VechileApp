...
app.get('/matka/all', (req, res, next) => {
...
app.get('/matka/one/:id', (req, res, next) => {
...
app.post('/matka/add',  upload.single('kuva'), (req, res, next) => {
	let matka = req.body;
  ...
  db.run('insert into matka (otsikko,paiva,paikka,saa,kuva,kuvaus) values (?, ?, ?, ?, ?, ?)',
	      [matka.otsikko, matka.paiva, matka.paikka, matka.saa, kuva, matka.kuvaus], (error, result) => {
		if (error) throw error;
...
app.get('/matka/delete/:id', (req, res, next) => {
	let id = req.params.id;
